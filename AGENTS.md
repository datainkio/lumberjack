# Agent Instructions for Lumberjack
...
- **Minimal changes**: Make focused, single-purpose edits. Avoid refactoring unrelated code unless explicitly requested.
- **Preserve style**: Match existing indentation, naming conventions, and code patterns in the file being modified.
- **Canonical source**: All modifications must target `src/` files. Never edit `dist/` directly; it is generated by `npm run build`.
- **ESM only**: All code must use ESM syntax (`import`/`export`). Do not introduce CommonJS patterns.

### File Modification Safety

- **Safe to modify**: `src/**/*.js`, `types.d.ts`, test files, documentation files (`.md`).
- **Read-only**: `dist/`, `node_modules/`, generated files, demo files (unless explicitly requested).
- **Verify before committing**: Run `npm run lint` to check syntax and `npm run sanity` to verify basic functionality.

### Debugging & Testing Approach

- **Isolate the problem**: Use `grep_search` and `code_search` to locate relevant code before making changes.
- **Add logging strategically**: Use lumberjack's own logging (with `enabled = true`) to trace execution flow when debugging.
- **Test changes**: Run relevant test commands (`npm run sanity`, `npm run lint`) to verify modifications don't break existing functionality.
- **Verify in context**: For browser features, test via `npm run demo` and inspect browser console. For Node features, test via scripts or test files.

### Decision-Making

- **Ask for clarification** when:
  - User intent is ambiguous (e.g., "fix this" without specifying the issue).
  - A change could affect multiple areas and trade-offs exist.
  - Modifying public API surface or breaking existing behavior.

- **Proceed independently** when:
  - Task is clearly scoped (e.g., "add logging to function X").
  - Bug fix is straightforward with clear root cause.
  - Documentation or non-breaking improvements are requested.

### Logging Best Practices for Agents

- **Brief mode** for status updates and quick checks (default).
- **Verbose mode** for debugging complex data structures or multi-step processes.
- **Silent mode** only for production-like scenarios; avoid in development.
- **Error style** automatically applied to `Error` objects; override only if different styling is needed.
- **Scoped loggers** for feature-specific output (e.g., build process, deployment steps).

### Documentation & Comments

- Do not add or remove comments unless explicitly requested.
- Update `.md` files only if documentation is outdated or incomplete.
- Prefer code clarity over comments; self-documenting code is preferred.
